...
def load_level(filename, dir="data_boss"):
    if dir == "none":
...
def load_image(name, dir="data_boss", colorkey=None, transform=None):
    if dir == "none":
...
def start_screen():
    ...
    fon = pygame.transform.scale(pygame.image.load('data_boss/fon.png'), (WIDTH, HEIGHT))
    screen.blit(fon, (0, 0))
...
def end_screen():
    ...
    pygame.mixer.music.load('data_boss/ds.mp3')
    pygame.mixer.music.play(1)
    fon = pygame.transform.scale(pygame.image.load('data_boss/gover.png'), (WIDTH, HEIGHT))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if 630 <= x <= 1230 and 520 <= y <= 720:
                    return 'cont'
                if 630 <= x <= 1230 and 770 <= y <= 970:
                    return 'back'
...
def victory_screen():
    pygame.mixer.music.stop()
    pygame.mixer.music.load('data_boss/bossout.mp3')
    pygame.mixer.music.play(1)
    fon = pygame.transform.scale(pygame.image.load('data_boss/yd.jpg'), (WIDTH, HEIGHT))
...
all_mobs.add(b)
state = True
pygame.mixer.music.load('data_boss/bm.mp3')
pygame.mixer.music.play(10)


def go():
    global running, game_over, time_passed, atk_timer, state, x, y, attack, mon_change_dir, moved_once,\
        boss_sprint_atk_timer, moved_once, moved, ox, ox1, oy, oy1, winning_score, all_sprites, all_objects,\
        all_mobs, all_booms, all_bombs, all_monsters, walls_group, lasers, coin,\
        projectiles, player_group, player_attacks, tiles_group, win, b, speed, surf, player
    ...
    while running:
        ...
        if not game_over:
            ...
            if state == True:
                ...
                fon = pygame.transform.scale(pygame.image.load('data_max/pause.png'), (WIDTH, HEIGHT))
                screen.blit(fon, (0, 0))
